...
- `agent:proof-failed`: terminal exact-head proof failed; a later passing proof removes this proof-owned blocker.
...
3. Triage deterministically seals the issue snapshot, priority, risk, proof level, acceptance clauses, anti-cheat probes, and optional proof route or interaction without a model call, then applies managed labels/comments and dispatches implementation.
   Intent digests seal stable policy labels while excluding transient `agent:triage` and `agent:implement` entry labels plus the `agent:proof-failed` result label, so lifecycle transitions cannot invalidate later exact-head gates.
...
10. After a trusted merge, automerge resolves the exact merge commit, dispatches baseline CI, CodeQL, and trusted Render verification for it, removes agent workflow labels, and closes the linked source issue while preserving priority labels.
    ...
    If the selected revision is absent, it requests the service's latest configured `main` branch without a commit pin and follows only the returned deployment.
    Success requires that deployed revision to contain the merge, remain on current `main`, and descend from the prior live revision.
...
An explicit manual proof route is valid only when every planned task uses that route; otherwise preparation fails before provider acquisition.
...
Intent capsule v7 canonicalizes the legacy `/staff/tasks` proof route to `/staff` while retaining exact v1 through v6 reconstruction across route, proof-result, and implementation-label transitions.
